...
function decryptSensitiveField(text) {
  ...
  try {
    if (!text.includes(':')) {
      if (text.startsWith('enc_')) return Buffer.from(text.replace('enc_', ''), 'base64').toString('utf8');
      return text;
    }
...
function privacyFirewallMiddleware(req, res, next) {
  ...
  const anonymizedIp = clientIp ? String(clientIp).replace(/\d+$/, 'xxx') : 'anonymized';

  console.log(`[PRIVACY FIREWALL] ${req.method} ${req.path} | Client: ${anonymizedIp} | Shield: AES-256 ACTIVE`);
